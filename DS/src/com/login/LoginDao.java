...
public class LoginDao {
	...
	public String checkUserLoginDetails(String loginId,String password,String userType, Map session2){
		...
		try{
			...
			Query query=session.createQuery("select ds.status,ds.password,ds.clientType,ds.blockStatus,ds.userId from LoginForm ds where ds.loginId=:loginId and ds.password=:password and ds.type=:type").setParameter("loginId",loginId).setParameter("password",password).setParameter("type",userType);
			...
			if(it.hasNext())
			{
				...
				if(dataPass.equals(password))
				{
					...
					String clientType=(String)row[2];
					...
					session2.put("clientType",clientType);
				}
				else
				{
					Status="invalid";
	...
	@SuppressWarnings("unchecked")
	public HashMap checkUserProfileStatus(String loginId,String password,String userType){

		Session session = HibernateSession.getSessionFactory().openSession();

		String Status="";
		HashMap DisDetailsmapInfo=new HashMap();
		...
		try{
			...
			Query query=session.createQuery("select dsd.distributorId,dsd.distributorInitial,dsd.companyName,dsp.clientId," +
					...
					"and dsd.distributorId=dsa.distributorId and dsd.mdId=dsm.mdId and dsm.clientId=dsp.clientId and dsl.loginId=:loginId " +
					"and dsl.password=:password and dsl.type=:type").setParameter("loginId",loginId).setParameter("password",password).setParameter("type",userType);
			...
			if(it.hasNext()){
				...
				DisDetailsmapInfo.put("totalCash",row[8]);
				DisDetailsmapInfo.put("usedCash",row[9]);
	...
	@SuppressWarnings("unchecked")
	public HashMap verifyEmail(String distributorId,String VerifiedCode){
		Session session = HibernateSession.getSessionFactory().openSession();
		Transaction tran=null;
		String code="";
	...
	@SuppressWarnings("unchecked")
	public HashMap getVerifyMobilrcode(String distributorId){
		Session session = HibernateSession.getSessionFactory().openSession();
		Transaction tran=null;
		String mobileCode="";
		String mobilenumber="";
