...
public class DistributorProfileDao {
	...
	public ArrayList<HashMap<String,String>> getDSJournal(String userId)
	{
		 
		ArrayList<HashMap<String,String>> ActiveAgnetInfo=new ArrayList<HashMap<String,String>>();
		Session session = null;
		Query query =null;
		try{
			session = HibernateSession.getSessionFactory().openSession();
			HashMap<String,String> temp;
			query=session.createSQLQuery("select top 10 mode_of_payment,ammount_to_credit,request_date,approval_date,Referece_no, status,remarks from distributor_journal where distributor_id='"+userId+"' order by request_date desc,request_time desc")
					...
					.addScalar("Referece_no", Hibernate.STRING).addScalar("status", Hibernate.STRING).addScalar("remarks", Hibernate.STRING);
