...
public class AgentDepositDao
{
	...
	public ArrayList<HashMap<String,Object>> getDepsitDetails(String userId) 
	{
		...
		try
		{
			/*Query depositRequestQuery=session.createQuery("select AJF.agentId,AJF.JournalId,AJF.journalDate,AJF.RequestTime,AJF.ModeOfPayment,AJF.AmounToCredit,AJF.ChequeNumber,ADF.agentInitial,ADF.agencyName,ADF.mobileNo,AJF.Bank_Charges from AgentJournalForm AJF,AgentDetailForm ADF where AJF.agentId=ADF.agentId and AJF.Status=:Status and AJF.distributorId=:userId order by AJF.RequestDate desc,AJF.RequestTime desc").setParameter("Status",status).setParameter("userId",userId) ;
			 ...
			 while(it.hasNext())
			 {
				...
				System.out.println("rowwwwwwwwwwwwwwwwwwwwwwww>>>>>>>>>>>>>>>>>>>>>>>"+row[2]);
	...
	public String getDsBalance(String userId) {
		...
		try{
			...
			//	amount=dff.format(totalCash-usedCash);

			/*			System.out.println("totalCash:  "+totalCash);
			System.out.println("usedCash:  "+usedCash);
			System.out.println("cutoffAamount:  "+cutoffAamount);*/

			System.out.println(" amount:  "+amount);

			/*Query query=session.createQuery("select dsa.totalCash,dsa.usedCash,dsa.cutoffAamount from DistributorAmountForm dsa where dsa.distributorId=:distributorId").setParameter("distributorId",userId);
	...
	{	
		...
		{	
			...
			System.out.println("agntAmt: "+amtOfCharege);
			...
			{        	
				...
				{		
					...
					/*		 System.out.println("agentid " +agentid);
					 ...
					 System.out.println("addeddAmount " +addeddAmount);
					 ...
					 System.out.println("Reamark  "+remark);*/
					...
					{					 
						...
						String textMessage="Rs."+amtOfCharege+" has been Credited to your ID: "+agentid+" (Bal: Rs."+AmountSend+"). Now SMS '"+SmsTranCode+"' to get details of last 3 transactions.";
						System.out.println(textMessage);
						SendSmsOnMobile.sendMobileSmsSMSZONE(AgentMobile,textMessage);
					}				
					CheckUpdate="Update";			 
				}
				else
				{				
					...
					if(ClientFlag.equalsIgnoreCase("1") || ClientFlag.equalsIgnoreCase("2"))
					{					
						String textMessage="TID-"+transactionNo+", your Request of Rs. "+transaferAmt+" has been Declined by your Distributor. Current BAL: Rs. "+ Math.floor(AgentCurrentBalance)+"";
						SendSmsOnMobile.sendMobileSmsSMSZONE(AgentMobile,textMessage);
					}
				}
	...
	public String updateAgentAccount(String userId, String agentid,
		...
		{			
			...
			if(ClientFlag.equalsIgnoreCase("1") || ClientFlag.equalsIgnoreCase("2"))
			{				
				String textMessage="TID-"+transactionNo+", your Request of Rs. "+transaferAmt+" has been Declined by Your Distributor. Current BAL: Rs. "+AgentCurrentBalance.toString()+"";
				System.out.println(" textMessage :: "+textMessage);
				SendSmsOnMobile.sendMobileSmsSMSZONE(AgentMobile,textMessage);//mobileNo
				//	}
			}	
			tran.commit();
		}
