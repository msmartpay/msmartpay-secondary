...
public class MoneyTransferDao {
...

	{
		...
		try {
			con = DBConnection.getConnection();
			pst = con.prepareStatement("select Reference_id from agent_transaction_details where agent_id="+Long.parseLong(agentID)+" and (Transaction_Id=? OR Transaction_No =? OR Reference_id=?)");
