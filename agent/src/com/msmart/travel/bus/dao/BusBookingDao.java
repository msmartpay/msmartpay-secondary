package com.msmart.travel.bus.dao;
...
public class BusBookingDao {
	...
	public String updateBusBookStatus(HashMap<String,Object> ticketDetailsMap,String UserTrackId) {
		...
		try
		{
			...
			String sql="update Bus_Booking_Details set Reference_Id=?,Transport_PNR=?,Departure_Time=?,"
			...
			pstmt.setString(4, transportDetails.get("Address1")==null?"":transportDetails.get("Address1").toString()+transportDetails.get("Address2")==null?"":transportDetails.get("Address2").toString());
			pstmt.setString(5, transportDetails.get("CityNamePinCode")==null?"":transportDetails.get("CityNamePinCode").toString());
			pstmt.setString(6, transportDetails.get("Fax")==null?"":transportDetails.get("Fax").toString());
			pstmt.setString(7, transportDetails.get("Website")==null?"":transportDetails.get("Website").toString());
			pstmt.setString(8, transportDetails.get("EmailId")==null?"":transportDetails.get("EmailId").toString());
			pstmt.setString(9, ticketDetailsMap.get("CancellationPolicy:")+"");
			pstmt.setString(10, ticketDetailsMap.get("UserTrackId")+"");

			pstmt.execute();

			pstmt.close();

			//Update live Recharge

			sql="Update live_recharge set status=? where USessionID=?";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, "Success");
			pstmt.setString(2, UserTrackId);
			pstmt.execute();

			pstmt.close();

			//Update Agent Transaction details

			sql="Update agent_transaction_details set tran_status=? where Reference_id=?";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, "Success");
			pstmt.setString(2, UserTrackId);
			pstmt.execute();
	...
	public String updateBusBookStatus(String UserTrackId,String tranId,long agentId) {
		...
		try
		{
			...
			cstmt = con.prepareCall("{call Bus_Booking_Update(?,?,?,?)}");

			cstmt.setLong(1, agentId);
			cstmt.setString(2, tranId);
			cstmt.setString(3, UserTrackId);
