...
public class TransactionAction extends ActionSupport{
	...
	public String InitiateTransaction ()
		...
		try
		{
			...
			JSONObject respJSON=api.getInitiateTransaction(SenderId,SenderName,BeneMobile, BeneAccount, BeneName, IFSC, TxnType,BankName,String.valueOf(TxnAmount),Remark,RecipientId,transactionId,tranKey,agentID);
	...
	public String verifyAccount()
		...
		try
		{
			...
			String REQUEST_ID="SK"+agentID+RandomStringUtils.randomNumeric(10);
	...
	public String ajaxverifyAccount()
		...
		try
		{
			...
			jsonobj =api.callApi("EAccountVerifyByBankAccountIFSC", fsJson);
			...
			logger.info("Verify status : "+jsonobj.toString());
		}catch (Exception ex) {
			logger.info("TEP :: Exception in AccountStatementAction class ");
			ex.printStackTrace();
			jsonobj=new JSONObject();
	...
	public String refundTransaction()
		...
		try
		{
			...
			request.setAttribute("requestType", "OTP");
	...
	public String verifyRefundTransaction()
		...
		try
		{
			...
			JSONObject respObj=dmrApi.getRefundOTP(SenderId, tranId,tranKey,agentID);
			if(respObj!=null && respObj.length()>0){
