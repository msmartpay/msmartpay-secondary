...

{
	...
	public String changePassword(String userId, String newPassword,String clientType) {
	...
	public HashMap<String,Object> forgetPassword(String email, String Client_Type) {
		...
		try 
		{
			...
			IdStatusQuery=session.createQuery("select DDF.distributorId,DDF.distributorInitial,LF.password,DDF.MobileNo,DDF.emailId from DistributorDetailForm DDF,LoginForm LF where DDF.distributorId=LF.userId and (LF.loginId=:email OR DDF.MobileNo=:mobile)").setParameter("email",email).setParameter("mobile",email);
			...
			if(it.hasNext())
			{
				...
				idStatus.put("mobileNo",row[3]);
				idStatus.put("emailId",row[4]);
