...
public final class ClientReportDao 
{
	...
	public final String getBillPaymentReport(String filePath, String reportOf, String portalId) 
	{
        ...
        try 
        {
			...
			sqlQuery="select count(a.user_id) from bill_mobile a where Date_of_Booking between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and a.user_id in (select agent_id from agent_details where distributor_id in(select distributor_id from distributor_details where md_id in(select md_id from md_details where client_id='"+portalId+"')))";
			...
			}else if(count>=1 && count<=20000)
			{
				...
				cstmt = con.prepareCall("{call Client_Admin_reports(?,?)}");
				cstmt.setString(1, reportOf);
				cstmt.setString(2, portalId);
	...
	public final  String getBusTranBookReport(String filePath, String reportOf, String portalId) 
	{
		session = HibernateSession.getSessionFactory().openSession();
        try 
        {
			con = session.connection();
			sqlQuery="select count(a.Agent_id) from redbus_book_cancel_details a where transaction_type='Purchase' and convert(varchar(10),D_O_transaction,120) between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and a.Agent_id in (select agent_initial+convert(varchar,agent_id) from agent_details where distributor_id in(select distributor_id from distributor_details where md_id in(select md_id from md_details where client_id='"+portalId+"')))";
			//System.out.println(sqlQuery);
			qry = session.createSQLQuery(sqlQuery);
			int count=Integer.parseInt(qry.uniqueResult().toString());
			//System.out.println(count);
			...
			}else if(count>=1 && count<=20000)
			{
				...
				writer.append("Service");
				writer.append(';');
				writer.append("Commsoft TicketNo");
				writer.append(';');
				writer.append("Gross Amount");
				writer.append(';');
	...
	public final String getBusTranCanReport(String filePath, String reportOf, String portalId)
	{
		session = HibernateSession.getSessionFactory().openSession();
        try 
        {
			con = session.connection();
			sqlQuery="select count(a.Agent_id) from redbus_book_cancel_details a where transaction_type='Cancellation' and convert(varchar(10),D_O_transaction,120) between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and a.Agent_id in (select agent_initial+convert(varchar,agent_id) from agent_details where distributor_id in(select distributor_id from distributor_details where md_id in(select md_id from md_details where client_id='"+portalId+"')))";
			//System.out.println(sqlQuery);
			qry = session.createSQLQuery(sqlQuery);
			int count=Integer.parseInt(qry.uniqueResult().toString());
			//System.out.println(count);
        ...
        {
			status = "ERROR";
			System.out.println("Exception in ClientReportDao,getBusTranCanReport");
			System.out.println(e.toString());
		} finally
		{
			try 
	...
	public final String getLICPremiumReport(String filePath,  String reportOf,  String portalId) 
	{
		session = HibernateSession.getSessionFactory().openSession();
		try
		{
			con = session.connection();
			sqlQuery="select count(user_id) from lic_premium where convert(varchar(10),Request_DATE,120) between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and user_id in (select agent_id from agent_details where distributor_id in(select distributor_id from distributor_details where md_id in(select md_id from md_details where client_id='"+portalId+"')))";
			//System.out.println(sqlQuery);
			qry = session.createSQLQuery(sqlQuery);
			int count=Integer.parseInt(qry.uniqueResult().toString());
			//System.out.println(count);
		...
		{
			status = "ERROR";
			System.out.println("Exception in ClientReportDao,getLICPremiumReport");
			System.out.println(e.toString());
		} finally
		{
			try 
	...
	public final  String getLiveRechargeReport(String filePath, String reportOf, String portalId)
	{
		session = HibernateSession.getSessionFactory().openSession();
		...
		{
			con = session.connection();
			sqlQuery="select count(user_id)  from live_recharge where date_of_recharge=convert(varchar(10),getdate()-1,120) and user_id in (select agent_id from agent_details where distributor_id in(select distributor_id from distributor_details where md_id in(select md_id from md_details where client_id='"+portalId+"')))";
			//System.out.println(sqlQuery);
			qry = session.createSQLQuery(sqlQuery);
			int count=Integer.parseInt(qry.uniqueResult().toString());
			//System.out.println(count);
	...
	public  final String getPanCardReport(String filePath, String reportOf, String portalId) 
	{
        ...
        {
        	con = session.connection();
        	sqlQuery="select count(user_id)  from pan_request where date_of_booking between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and user_id in (select agent_id from agent_details where distributor_id in(select distributor_id from distributor_details where md_id in(select md_id from md_details where client_id='"+portalId+"')))";
			//System.out.println(sqlQuery);
			qry = session.createSQLQuery(sqlQuery);
			int count=Integer.parseInt(qry.uniqueResult().toString());
			//System.out.println(count);
        ...
        {
			status = "ERROR";
			System.out.println("Exception in ClientReportDao,getPanCardReport");
			System.out.println(e.toString());
		} finally 
		{
			try 
	...
	public final String getSuspectDataReport(String filePath, String reportOf, String portalId) 
	{
		...
		{
			con = session.connection();
			sqlQuery="select count(user_id) from live_recharge where convert(varchar(10),date_of_suspect,120) between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and suspect_flag='1' and user_id in (select agent_id from agent_details where distributor_id in (select distributor_id from distributor_details where md_id in (select md_id from md_details where client_id='"+portalId+"')))";
			//System.out.println(sqlQuery);
			qry = session.createSQLQuery(sqlQuery);
			int count=Integer.parseInt(qry.uniqueResult().toString());
			//System.out.println(count);
		...
		{
			status = "ERROR";
			System.out.println("Exception in ClientReportDao,getSuspectDataReport");
			System.out.println(e.toString());
		} finally 
		{
			try 
	...
	public String getAirDomesticAccountReport(String filePath, String portalId) 
	{
		Connection con = null;
		Statement stmt = null;
		ResultSet rs = null;
...
//			 " dom_air_flight_segments where convert(varchar(10),trans_creation_date,120) between '" + fromDate + "' and '" + toDate + "' order by fk_orgn_dstn_id";
			...
			sqlQuery="select CONVERT(VARCHAR(11),daf.trans_creation_date,106) as booking_date, CONVERT(VARCHAR(11),daf.trans_creation_date,108) as booking_time," +
					...
					"(select agent_id from agent_details where distributor_id in (select distributor_id from distributor_details where md_id in(" +
					"select md_id from md_details where client_id in (select portal_id from admin_user_details where user_id='"+portalId+"')))) order by fk_orgn_dstn_id";
			//System.out.println(sqlQuery);
			stmt = con.createStatement();
...
//				writer.append("");
	...
	public   String getAirDomesticCancellationAccountReport(String filePath, String portalId) 
	{
		Connection con = null;
		Statement stmt = null;
		ResultSet rs = null;
...
//System.out.println("    sql query for getting data from Air ticket details is :::    " + sqlQuery);
			sqlQuery="select distinct dot.cancelation_id, dot.partner_ref_id from dom_air_ticket_details dot,dom_air_bookings dab where cancelation_id is not null and (dot.cancelation_status='Processed' or dot.cancelation_status_return='Processed' or dot.cancelation_status_depart='Processed')and convert(varchar(10),dot.trans_creation_date,120) between convert(varchar(10),getdate()-7,120) and convert(varchar(10),getdate(),120) and dot.partner_ref_id=dab.partner_ref_id and dab.agent_id in (select agent_id from agent_details where distributor_id in (select distributor_id from distributor_details where md_id in (select md_id from md_details where client_id in (select portal_id from admin_user_details where user_id='"+portalId+"') ))) order by dot.cancelation_id desc";
//			System.out.println(sqlQuery);
			stmt = con.createStatement();
			rs = stmt.executeQuery(sqlQuery);
			while(rs.next())
...
//						.getClosingBalance()));
		...
		{
			status = "norecord";
			System.out.println("Exception in ClientReportDao,getAirDomesticCancellationAccountReport");
			System.out.println(e.toString());
		}finally {
			try {
				con.close();
